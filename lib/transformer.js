...
class Transformer {
	...
	transformRootFile(rootFileName, mdFilesContents) {
		...
		mdFilesContents.forEach((mdFileContent) => {
			...
			mdFileContent.questions.forEach(q => readMeContents.push(mdFileContent.fileName + q.anchor));
			readMeContents.push("");
	...
	transform(mdFile) {
		...
		let newFileContent = pr.replaceTableOfContent(fileContent, questions.map(q => q.anchor)).join(EOL);
